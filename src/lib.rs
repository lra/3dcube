// Shared renderer: cube data, shading, culling. Native front end in main.rs,
// wasm front end in wasm.rs + web/index.html.
mod math;
mod raster;
#[cfg(target_arch = "wasm32")]
mod wasm;

use math::{V3, cross, dot, rot, sub};
use raster::fill_quad;
pub use raster::{H, W};
...
/// Render one frame at animation time `t` into `buf` (W*H 0RGB pixels).
pub fn render(t: f32, buf: &mut [u32]) {
    let light = {
